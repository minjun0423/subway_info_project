...
def search_station(station: str = Form(...)):
    return RedirectResponse(url=f"/subway/{station}", status_code=303)

@app.get("/subway/{station}", response_class=HTMLResponse)
def get_real_time_arrival_info(request: Request, station: str):
    try:
        ...
        for arrival_info in data["realtimeArrivalList"]:
            station_name = arrival_info["statnNm"]
        ...
        return templates.TemplateResponse("subway.html", {
            "request": request,
            "station": station_name,
            "arrival_info": processed_data
        })
